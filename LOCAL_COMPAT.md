...
本次完整离线回归发现 67 个用例，63 个通过，4 个独立 MySQL 用例因未启用一次性测试数据库而跳过。新增自动群状态查询的非阻塞、去重限频、失败不误判模式、已知目标不查询与卸载取消用例。覆盖真实 SDK 模拟登录后的原生 parser、ws_dispatch、两个群 callback→同步 commit→create_event、真实 CommandFilter、数字 OpenID 前导零、并发/取消、钩子重复安装及卸载恢复、诊断敏感字段排除与只读 GET 模拟。测试未执行线上额度变更或绑定写入。新诊断用于下一条真实消息，不能以合成测试代替实际群的成员身份下发验收。
